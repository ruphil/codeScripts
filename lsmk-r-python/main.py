...
def searchflights():
    ...
    flightdate = request.args['flightdate']
    flownflightstatus = request.args['flownflightstatus']

    range_name = 'register!A2:AM3000'
    ...
    for index, row in enumerate(rows):
        if(row[0] == dronenumber and row[3] == flightdate and row[36] == flownflightstatus):
...
def dfsubmit():
    ...
    result = service.spreadsheets().values().append(spreadsheetId=dfsubmit_log_sheet_ID, range='register!A:AM', valueInputOption='RAW', body=body).execute()
    result.get('updates')
    return "success"
...
def dtsubmit():
    ...
    range = 'register!AM' + str(flightIndexInSheet) + ':AZ' + str(flightIndexInSheet)
    print(range)
